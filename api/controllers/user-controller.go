...
import (
	...
	"github.com/imsujan276/untold-story/api/auth"
	"github.com/imsujan276/untold-story/api/models"
	"github.com/imsujan276/untold-story/api/security"
...
func (server *Server) UpdateAvatar(c *gin.Context) {
	...
	err = server.DB.Debug().Model(models.User{}).Where("id = ?", uid).Take(&tempUser).Error
	if err != nil {
		errList["Not_Found"] = fmt.Sprintf("User not found: %v", uid)
		c.JSON(http.StatusNotFound, gin.H{
			"status": http.StatusUnprocessableEntity,
			"error":  errList,
		})
	...
	// filePath := utilities.UniqueFileName(file.Filename)
	filePath := utilities.CustomFileName(file.Filename, fmt.Sprint(uid)+"-"+fmt.Sprint(tempUser.Username))
	profilePhotospath := "static/images/user/" + filePath

	// remove existing file if any
	_ = os.Remove(profilePhotospath)

	// Local storage save
	// err = c.SaveUploadedFile(file, profilePhotospath)
	err = utilities.SaveUploadedFile(file, profilePhotospath)
	if err != nil {
		errList["Cannot_Upload"] = err.Error()
		c.JSON(http.StatusInternalServerError, gin.H{
			"status": http.StatusInternalServerError,
			"error":  errList,
		})
	...
	user := models.User{}
	user.AvatarPath = profilePhotospath
	user.Prepare()
...
func (server *Server) UpdateUser(c *gin.Context) {
	...
	if err != nil {
		errList = utilities.FormatError(err.Error())
		c.JSON(http.StatusInternalServerError, gin.H{
			"status": http.StatusInternalServerError,
			"error":  errList,
		})
