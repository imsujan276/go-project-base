...
func (s *sendMail) SendResetPassword(ToUser string, FromAdmin string, Token string, Sendgridkey string, AppEnv string) (*EmailResponse, error) {
	...
	if os.Getenv("APP_ENV") == "production" {
		forgotUrl = "https://localhost.com/resetpassword/" + Token //this is the url of the frontend app
	} else {
	...
	email := hermes.Email{
		Body: hermes.Body{
			Name: ToUser,
			Intros: []string{
				fmt.Sprintf("Welcome to %s! Good to have you here.", os.Getenv("APP_NAME")),
			},
	...
	from := mail.NewEmail(os.Getenv("APP_NAME"), FromAdmin)
